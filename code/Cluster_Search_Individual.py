...
machine_image='ami-f44a869c'#'ami-2433ff4c'#'ami-7341831a'
...
entity_names = pd.DataFrame(%(entity_names)r,
                    columns=["Name"],
                    index=%(index)r) #pd.read_csv('../Data/Entities/all_entities.csv',index_col=0)
...
dirlist = listdir("../Data/Entities/Search_Results/")
...
for start, stop in zip(entity_block_starts, entity_block_stops):
    ...
    instance = launch_instance(user_data=startup_script%parameters,
                                    ...
                                    tag='world_bank_query_%i,%i'%(start,stop))
    ...
    while not cmd:
        if attempt_count>10:
            raise Exception("Can't connect to this instance no matter how hard I try! %i,%i"%(start,stop))
        print("Attempt %i to connect"%attempt_count)
        try:
            cmd = boto.manage.cmdshell.sshclient_from_instance(instance,
                                                                  key_path,
                                                                  user_name=login_user)
            cmd.get_file("all_entities_Google_results_%i_to_%i.csv"%(start,stop),"../Data/Entities/Search_Results/all_entities_Google_results_%i_to_%i.csv"%(start,stop))
            cmd.get_file("all_entities_Google_results_%i_to_%i.h5"%(start,stop),"../Data/Entities/Search_Results/all_entities_Google_results_%i_to_%i.h5"%(start,stop))
        except:
            attempt_count+=1
            pass
    instance.terminate()
